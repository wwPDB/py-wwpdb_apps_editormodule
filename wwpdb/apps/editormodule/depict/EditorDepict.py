...
class EditorDepict(object):
    ...
    def __init__(self,verbose=False,log=sys.stderr):
        ...
        self.__navTabGroupsDefault = [
                        ...
                        {"id": "citation",
                         "dsply_lbl": "Citation",
                         "categories": ["citation","citation_author"]
                        },
                        {"id": "advsry_rmrks",
                         "dsply_lbl": "Advisory and Remarks",
                         "categories": ["database_PDB_caveat","pdbx_entry_details","refine","struct_keywords","struct_biol"]
                        },
                        {"id": "polymr_desc",
                         "dsply_lbl": "Polymer Description",
                         "categories": ["entity","entity_name_com","entity_src_gen","entity_src_nat","entity_src_syn"]
                        },
                        {"id": "crystlztn",
                         "dsply_lbl": "Crystallization",
                         "categories": ["exptl_crystal_grow"]
                        },
                        {"id": "crystl_data",
                         "dsply_lbl": "Crystal Data",
                         "categories": ["diffrn","diffrn_detector","diffrn_radiation","diffrn_source","exptl_crystal"]
                        },
                        {"id": "data_collect",
                         "dsply_lbl": "Data Collection",
                         "categories": ["reflns","reflns_shell"]
                        },
                        {"id": "refine",
                         "dsply_lbl": "Refinement",
                         "categories": ["refine","refine_ls_shell"]
                        },
                        {"id": "links",
                         "dsply_lbl": "Links",
                         "categories": ["struct_conn"]
                        },
    ...
    def getJsonDataTable( self, p_reqObj, p_ctgryRcrdList, p_iDisplayStart, p_ctgryColList ):
        ...
        if( self.__verbose and self.__debug ):
            self.__lfh.write("+%s.%s() -- DEBUG -- aaDataList after call to createDataTableAaDataList is: %r \n" % (self.__class__.__name__,
                                                                              sys._getframe().f_code.co_name,
                                                                              aaDataList) )
...
<div class="ui-state-highlight ui-corner-all noprint">
    <p>Color legend: <span class="ui-icon ui-icon-info fltlft"></span> <span class="prmry_key smallbox">Primary Key Column</span> <span class="newrecord smallbox">New Row Added by User</span> <span class="row_selected smallbox">Row Selected for Action</span><br class="clearfloat">
    </p>
</div>
'''
        if labelName:
        ...
        if( bOk ):
            if( len(catObjDict['COLUMN_DISPLAY_ORDER']) > 0):
                ...
                catObjDict['MANDATORY_COLUMNS'] = catObjDict['MANDATORY_COLUMNS_ALT'] if( catObjDict['MANDATORY_COLUMNS_ALT'] and len(catObjDict['MANDATORY_COLUMNS_ALT']) > 0 ) else \
                ...
                mrkpList.append('<br class="clearfloat"/><br /><div id="'+p_cifCtgryNm+'_record_actions"><input style="display: inline; font-size: .8em;" class="cifctgry_add_row '+p_cifCtgryNm+'_row_action" id="'+p_cifCtgryNm+'_add_row" name="'+p_cifCtgryNm+'" value="Add Record" type="button">')
                mrkpList.append('<span class="select_for_action help" id="select_for_action_'+p_cifCtgryNm+'" style="padding-left: 20px;"><strong>RIGHT-click (or CTRL-click) on row to mark respective record for action.</strong></span>')
                mrkpList.append('<span class="unselect help" id="unselect_'+p_cifCtgryNm+'" style="padding-left: 20px;"><strong>RIGHT-click (or CTRL-click) on row to unmark selected record for action.</strong></span>')
                if( context != "editorconfig" ):
                    mrkpList.append('<input style="display: inline; margin-left: 20px;  font-size: .8em;" class="cifctgry_delete_row '+p_cifCtgryNm+'_row_action onrowselect" id="'+p_cifCtgryNm+'_delete_row" name="'+p_cifCtgryNm+'" value="Delete Record" type="button">')
                #mrkpList.append('<input style="display: inline; margin-left: 20px;  font-size: .8em;" class="cifctgry_insert_row '+p_cifCtgryNm+'_row_action onrowselect" id="'+p_cifCtgryNm+'_insert_row" name="'+p_cifCtgryNm+'" value="Insert Record After" type="button">')
                mrkpList.append('<input style="display: inline; margin-left: 20px;  font-size: .8em;" class="jmol_view '+p_cifCtgryNm+'_row_action onrowselect" id="'+p_cifCtgryNm+'_jmol_view" name="'+p_cifCtgryNm+'" value="Load Record Into Viewer" type="button">')
                mrkpList.append('</div>')
            else:
    ...
    def __configureInputTypes(self,p_cifCtgryNm,p_catObjDict,p_ctgryColList):
        ...
        for idx,type in colTypes.items():
            ...
            inputTypes[idx] = 'text'
            if( idx < len(p_ctgryColList) ):    # not sure why yet, but had to enforce this check for category such as "atom_site" where index of "1" actually skipped, 
                ...
                if( type == 'select' or (len(enumOpts) > 0 and idx in enumOpts) ):
                    ...
                    """
                    if( p_cifCtgryNm+"."+p_ctgryColList[idx] in ['struct_keywords.pdbx_keywords', 'pdbx_reference_molecule.class', 'exptl_crystal_grow.method',
                                                                 'pdbx_nmr_exptl.type', 'pdbx_nmr_exptl_sample.isotopic_labeling',
                                                                 'pdbx_nmr_sample_details.solvent_system', 'pdbx_audit_support.funding_organization',
                                                                 'pdbx_nmr_ensemble.conformer_selection_criteria'] ):
                            #inputTypes[idx] = 'select'
                            inputTypes[idx] = 'select_w_other'
                    elif( len(enumOpts[idx]) > 35 ):
                        if( p_cifCtgryNm in ['entity_src_gen','entity_src_nat'] and  'scientific' in p_ctgryColList[idx] ) or ( p_cifCtgryNm+"."+p_ctgryColList[idx] == 'software.name' ):
    ...
    def __genCtgryNavBar(self,p_dataBlockName,p_fileSource,p_dataFile,p_bIsWorkflow,p_context):
        ...
        drpDwnMrkpTmplt_single=r'''<li><a href="#" class="cifctgry_submit ctgrygrpid_%(ctgrygrpid)s %(cardinality)s" id="submit-%(selectid)s" name="%(ctgry)s" style="font-size: %(font_size)s;" >%(drpdwn_displ_lbl)s</a></li>'''
        
        drpDwnMrkpTmplt_combo=r'''<li><a href="#" class="multi_cifctgry_submit ctgrygrpid_%(ctgrygrpid)s %(cardinality)s" id="%(ctgry)s" name="%(ctgry_disply_lbls)s" >%(drpdwn_displ_lbl)s</a></li>'''
                ...
                for dropDwnDisplLbl,ctgryDict in navTabGrp['dropdown_display_labels']:
                    ...
                    if( len( ctgryTuplLst ) > 1 ):
                        ...
                        for idx,ctgryTupl in enumerate(ctgryTuplLst):
                            if( self.__debug ):
                                self.__lfh.write("%s.%s() ------ DEBUG ------ combo drop-down choice found. ctgryTupl is: %r\n" % (self.__class__.__name__, sys._getframe().f_code.co_name, ctgryTupl) )
                            separator=''
                            if( idx > 0 ): separator = '+'
